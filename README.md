# LaTeX author support for the International Press journal *Mathematics, Computation and Geometry of Data* (*MCGD*)
...
* `mcgd-template.tex` - topmatter template (should be used for article preparation);
* `mcgd-sample.tex` - journal sample article;
* `mcgd-sample.pdf` - journal sample article (`PDF` file);
...
* Use the file `mcgd-template.tex` to start your article as a template.
* Use the file `mcgd-sample.tex` as a reference for how to prepare a topmatter of your article.

## Recommended usage of `ipart` package

Use `mcgd-template.tex` as a template.

### Document class options

For the MCGD journal `mcgd` option must be set
in a `\documentclass[]{ipart}`:
```latex
\documentclass[mcgd]{ipart}
```
...
```latex
\documentclass[mcgd,authoryear]{ipart}
```
...
```latex
\documentclass[mcgd,nonatbib]{ipart}
```
...
[github](https://github.com/vtex-soft/texsupport.intlpress-mcgd/issues) page.